...
import { Plus, UserPlus, Play, Square, CheckSquare, Users, Trophy } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        // Fetch users and stats for read-only view
        const fetchData = async () => {
            try {
                const usersRes = await api.get('/users');
                // Filter to show ONLY voters for the Admin view
                const votersOnly = usersRes.data.filter(u => u.role === 'voter');
                setUsers(votersOnly);

                const statsRes = await api.get('/users/stats');
                setStats(statsRes.data);
            } catch (error) {
                console.error("Failed to fetch user data for admin view", error);
            }
        };
        fetchData();
            ...
            <div className="container mx-auto px-4 py-8">
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {election.status === 'completed' && (
                                                <Link to={`/results/${election._id}`} className="bg-blue-50 text-blue-600 px-3 py-1 rounded hover:bg-blue-100 flex items-center gap-1 text-sm font-medium">
                ...
                {activeTab === 'voters' && (
                    <div className="animate-fade-in">
                        {stats && (
                            <div className="bg-white p-6 rounded-lg shadow-md mb-8 border-l-4 border-blue-500 flex items-center justify-between">
                                <div>
                                    <h3 className="text-gray-500 text-sm font-uppercase">Total Voters</h3>
                        ...
                        <div className="bg-white rounded-lg shadow-md overflow-hidden">
                            <div className="p-6 border-b border-gray-200">
                                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                                    <Users className="h-6 w-6 text-gray-600" /> Voter List (Read-Only)
                                </h2>
                            </div>
                            <UserTable
                                users={users}
